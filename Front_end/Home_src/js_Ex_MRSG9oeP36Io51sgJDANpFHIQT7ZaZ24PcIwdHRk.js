 ...
 * https://modernizr.com/download/?-cssanimations-csstransitions-inputtypes-objectfit-domprefixes-prefixed-prefixes-setclasses-shiv-testallprops-testprop-teststyles !*/
	...
	$.DLMenu.prototype = {
		...
		_config : function() {
			...
			this.$el.find('ul.dl-submenu').each(function() {
  			$(this).prepend('<li class="dl-back"><a href="#">back</a></li>');
  		});
...
(function($){
  $(window).on('load', function() {
    ...
    function scrollToHash(hash) {
      if ( hash.match(/^#article\-carousel\-/) || hash.match(/^#documents\-filter/) || hash.match(/^#publications\-filter/) || hash.match(/^#hero\-carousel\-/) || hash.match(/^#tab\-/) ) {
        return false;
      }
      ...
      try {
        ...
        if ($target.length) {
          ...
          var bodyOffset = (bodyPaddingTop) ? parseInt(bodyPaddingTop) : 0;
          ...
          if (hash.match(/^#cu_accordion_item/)) {
            $target.find('button.accordion').addClass("active").next().slideDown(200, function() {
              $(this).addClass("show");
            });
          }
          if ($target.parents(".paragraph--type--cu-accordion-item").length) {
            $target.parents(".paragraph--type--cu-accordion-item").find('button.accordion').addClass("active").next().slideDown(200, function() {
              $(this).addClass("show");

              topOffset = $target.offset().top - bodyOffset;
        ...
        console.log("scrollToHash() error: ", err);
        if (typeof ga == 'function') {
          ga('send', 'event', 'scrollToHash(hash) Error', 'error: ' + err);
        }
      }
    }
...
if (!cookie) {
  document.write('<div id="cu-privacy-notice"><div id="cu-privacy-notice-wrapper"><button class="cu-privacy-notice-close" onClick="cuPivacyNoticeClose()" aria-describedby="cu-privacy-notice-text" tabindex="0"><span class="sr-only">Close Cookie Notice</span><div id="cu-privacy-notice-button">I AGREE</div></button><div id="cu-privacy-notice-text">' + message + '</div></div></div></div>');
...
(function($){
  $(document).ready(function() {
    if ($("body.page-node-type-cu-landing-page").length) {
      if (!$("section.cu_publications-block_3 .views-row").length) {
        $("section.cu_publications-block_3").addClass("js-empty");
      }
      if (!$("section.research_projects-block_2 .views-row").length) {
        $("section.research_projects-block_2").addClass("js-empty");
      }
      if (!$("section.gallery-block_2 .image-tile").length) {
        $("section.gallery-block_2").addClass("js-empty");
      }
      if ($(".banner-image").length) {
        if ($(".banner-image").html().trim() == "") {
          $(".banner-image").addClass("js-empty");
        }
      }
      if (!$("section.people-block_1 .views-row").length) {
        $("section.people-block_1").addClass("js-empty");
      }
      if (!$("section.people-block_2 .views-row").length) {
        $("section.people-block_2").addClass("js-empty");
      }
      if (!$("section.cu_publications-block_1 #publications-listing").length) {
        $("section.cu_publications-block_1").addClass("js-empty");
      }
      if (!$("section.research_projects-block_1 .views-row").length) {
        $("section.research_projects-block_1").addClass("js-empty");
      }
      if (!$("section.gallery-block_1 .image-tile").length) {
        $("section.gallery-block_1").addClass("js-empty");
      }
      if (!$("section.recent_news-block_1 .views-row").length) {
        $("section.recent_news-block_1").addClass("js-empty");
      }
      if (!$("section.recent_news-block_3 .views-row").length) {
        $("section.recent_news-block_3 > .container").addClass("js-empty");
      }
      if (!$("section.cu_events-block_1 .views-row").length) {
        $("section.cu_events-block_1 > .container").addClass("js-empty");
      }
...
angular.module('app').requires.push('ngCookies');

app.directive("cuAlerts", ['cuLocalAlertsFactory', '$cookies', function(cuLocalAlertsFactory, $cookies){
  return {
    restrict : "A",
    link: function (scope, element, attrs) {
      ...
      scope.is_admin = 0;
      if (jQuery("body").hasClass("site-admin")) {
        scope.is_admin = 1;
      }

      cuLocalAlertsFactory.getFeed(scope.url).then(function(data) {
        for (var i in data) {
          var block_id = "alert-" + data[i].id + '-' + data[i].changed;
          
          if (scope.cookies[block_id] !== undefined) {
            data[i].hide = 1;
          }
          else {
            scope.notEmpty = 1;
      ...
      scope.closeAlert = function(alert) {
        var block_id = "alert-" + alert.id + '-' + alert.changed;
        $cookies.put(block_id, block_id, { path: '/' });

        var target = jQuery("#alert-" + alert.id);
    ...
    template:
      '<div id="local-alerts" ng-if="localAlerts">' +
        '<div class="container">' +
          '<div id="alert-{{ alert.id }}" class="js-hide local-alert cu-alert" ng-repeat="alert in localAlerts" ng-if="!alert.hide || is_admin" slide-down>' +
            '<div class="alert-inner">' +
              '<div ng-bind-html="::getIcon(alert.attr)"></div>' + 
              '<div class="content">' +
                '<span class="glyphicon glyphicon-remove-sign pull-right" tabindex="0" ng-if="::!is_admin" ng-click="closeAlert(alert)"></span>' +
                '<h2 ng-switch on="alert.featured || alert.content !== \'\' || \'null\'">' +
                  '<span ng-switch-when="null" ng-bind-html="alert.title"></span>' +
                  '<a ng-switch-default href="{{ alert.path }}" ng-bind-html="alert.title"></a>' +
                  '<span class="edit-link badge" ng-if="::is_admin"><a ng-href="/node/{{ alert.id }}/edit">edit</a></span>' +
                '</h2>' +
                '<div ng-bind-html="alert.summary"></div>' +
              '</div>' +
            '</div>' +
          '</div>' +
        '</div>' +
...
app.factory('cuLocalAlertsFactory', function($http) {
  return {
    getFeed: function(target) {
      return $http({
        url: drupalSettings.path.baseUrl + 'json/alerts/local?_format=json',
        method: "GET",
        params: {url: target},
        timeout: 5000,
...
(function ($, drupalSettings) {
  ...
  function bindCloseAlert() {
    // Hide alert click event.
    $(".close-alert").off("click").on("click", function() {
      var alert = $(this).parents('.cu-alert');
      alert.slideUp(200, function() {
        alert.remove();
      });

      var block_id = alert.attr('id');
      // Set the alert cookie.
      $.cookie(block_id, block_id, { path: '/' });
    });        
  }

  if (drupalSettings.cu_alert && drupalSettings.cu_alert.master !== undefined) {

    var master = drupalSettings.cu_alert.master;
    
    if (master) {
      if (master != "none") {
        master = location.protocol + '//' + master + '/alerts/data';
        $.ajax({
          url: drupalSettings.path.baseUrl + "modules/custom/columbia/cu_utility_functions/json-wrapper.php",
          data: "url=" + master
        }).done(function(data) {
          if (data) {
            ...
            var total_alerts = 0;
            total_alerts =  Object.keys(data).length;
            
            // Iterate over responses and build alert html.
            $.each(data, function(index, alert) {
              // Check if cookie has been set for the given alert. Cookie id is set based on the alert id
              // in the form {alert id}, eg: 'alert-20'.
              alert = '<div>' + alert + '</div>';
              
              var $alert;
              $alert = $(alert).find(".cu-alert");

              var alert_id;
              alert_id = $alert.attr('id');
              var block_id = alert_id;

              if (!is_admin) {
                if (cookies[block_id] === undefined) {
                  alerts += $alert.wrap('<div/>').parent().html();
                }
              }
              else {
                if (cookies[block_id] !== undefined) {
            ...
            if (alerts) {
              var markup = "";
              markup = '<div id="global-alerts"><div class="container">' + alerts + '</div></div>';
              $('#alerts').html(markup).slideDown(500);
            }
